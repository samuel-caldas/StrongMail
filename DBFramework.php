...
<?php
//conectar
function conect($server,$usr,$pass,$banco){if($cnx=mysql_connect($server,$usr,$pas)){if(mysql_select_db($banco,$cnx)){return true;}else{return false;}}else{return false;}}
//inserir
...
function update($tabela,$val,$wr){
	$val=implode(', ', $val);
	$wr=implode(', ', $wr);
	if(mysql_query("UPDATE ".$tabela." SET ".$val." WHERE ".$wr)){return true;}else{return false;}
};
//seleciona
function select($wt, $tab, $wr, $order){		
if($wr){$wr=" WHERE ".$wr;}; if(!$wt){$wt="*";}; if($order){$order=" ORDER BY ".$order;}; 
...
conect('localhost','root','','strongmail');
?>